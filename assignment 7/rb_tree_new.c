...
void double_black(node** root_ptr, node* x)
{
    while(x != *root_ptr && colour(x)==false)
    {
        node* w = sibling(x);
        node* r = NULL;
        node* p = x->parent;
        ...
        if(x == p->left)
        {
            ...
            if(colour(w->right) == false && colour(w->left) == false)
            {
                w->colour = true;
                printf("x=x->parent.....(%d to ",x->value);
                x = x->parent;
                printf("%d)\n",x->value);
            }
            else 
            {
                if(colour(w->right) == false)
                {
                    (w->left)->colour = false;
                    w->colour = true;
    ...
    if(x!=NULL)
        printf("after loop x=%d, colour= %s",x->value, colour(x)?"red":"false");
    else
        printf("after loop x=NULL");
    x->colour = false;

    if(x!=NULL)
        printf("after double_black x=%d, colour= %s",x->value, colour(x)?"red":"false");
    else
        printf("after double_black x=NULL");
}   
